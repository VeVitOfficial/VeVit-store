...
$storage=new LocalPrivateStorage($root,['image/jpeg'],1024,null);$stored=$storage->store($tmp,'photo.php.jpg');test_assert_same('image/jpeg',$stored['mime'],'MIME comes from finfo');test_assert_true(!str_contains($stored['key'],'photo'),'storage key excludes original name');test_assert_true(is_file($storage->resolve($stored['key'])),'stored file resolves inside root');
